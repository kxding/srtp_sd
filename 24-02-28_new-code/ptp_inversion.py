...
def default_score_estimation(model,latents,context,t,guidance_scale,do_classifier_free_guidance):
    ...
    print("unet.latent = ", latent_model_input.shape)
    print("unet.t = ", t)
    print("unet.embed = ", context.shape)
...
class Inversion(DiffusionPipeline):
    ...
    def backward_diffusion_ptp(self,
        ...
        optimizer = SGD([latents], lr=0.1)
        dds_loss = DDSLoss(device=model.device, pipe=ldm_stable)
        init_latent = torch.cat([init_latent, init_latent], dim=0)
        with torch.enable_grad():
            for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
                optimizer.zero_grad()
                if uncond_embeddings_ is None:
                    context = torch.cat([uncond_embeddings[i].expand(*text_embeddings.shape).to(model.device), text_embeddings])
                else:
                    context = torch.cat([uncond_embeddings_, text_embeddings])
