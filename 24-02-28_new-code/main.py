...
def multi_concept_inversion():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                # mask = 1 if timesteps % 100 == 0 else 0
                if args.use_ref_cfg:
                    mask = torch.where(timesteps % 100 < int(100 * args.ref_cfg_ratio),
                                       torch.tensor(1, device=latents.device), torch.tensor(0, device=latents.device))
                ...
                # cfg_pred = src_pred + 7.5 * (ref_pred - src_pred)
                # cfg_pred = unet(noisy_latents, timesteps, encoder_hidden_states).sample
                ...
                # l2_target = target + 7.5/(7.5 + 1) * (src_pred - target)
                if args.use_ref_cfg:
                    cfg_ratio = args.guidance_scale_train_ref
                    l2_target = target + cfg_ratio / (cfg_ratio + 1) * (src_pred - target)
                    loss += mask * F.mse_loss(ref_pred.float(), l2_target.float(), reduction="none")
                loss = loss.mean()
    ...
    return src_reversed_latent_list, src_embedding_list, src_file_name
...
if __name__ == "__main__":
    ...
    if args.use_prev_embeddings != True:
        src_reversed_latent_list, src_embedding_list, src_file_name= multi_concept_inversion()
    else:
        ...
        src_file_name = str(args.src_embedding_path).rstrip('/')
        src_file_name = src_file_name.split('/')[-1]
        args.output_dir = args.src_embedding_path

    now = datetime.datetime.now()
    time_smp = str(now).replace(" ", "_").replace(".", "-")
    ref_file_name = get_image_file(args.concept_image_dir).split('.')[0]
    os.makedirs(args.output_image_path, exist_ok=True)
    print("cross_attention_ratio = ", args.cross_attention_injection_ratio)
    print("self_attention_ratio = ", args.self_attention_injection_ratio)
    # gen_image.gen_image_dds(
    #     pretrained_model_name_or_path=args.pretrained_model_name_or_path,
    #     output_img_path_name=args.output_image_path + "/" + src_file_name + "_" + ref_file_name + "_" + str(args.guidance_scale_train_ref) + "_Nocontrol-" + str(args.no_controller) + "_DI-" + str(args.use_direct_inversion) + "_" + str(args.cross_attention_injection_ratio) + "_" + str(args.self_attention_injection_ratio) + "_`" + time_smp + ".png",
    #     backward_prompts=["", "<s2>"],
