...
def parse_args():
    ...
    parser.add_argument(
        "--concept_image_dir", 
        type=str, 
        default='./examples/concept_image/', 
        required=True, 
    ...
    parser.add_argument(
        "--content_image_dir", 
        type=str, 
        default='./examples/content_image/', 
        required=True, 
...
def GetVsrc(tokenizer, text_encoder, unet, vae, noise_scheduler, placeholder_token):
    add_tokens(tokenizer, text_encoder, placeholder_token, args.concept_embedding_num, args.initializer_token)
    ...
    train_dataset = InversionDataset(
        data_root=args.content_image_dir,
        tokenizer=tokenizer,
...
def multi_concept_inversion(tokenizer, text_encoder, unet, vae, noise_scheduler, placeholder_token):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                ldm_stable = StableDiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path, scheduler=scheduler, tokenizer=tokenizer, text_encoder=text_encoder).to("cuda")
                ...
                encoder_hidden_states = text_encoder(batch["input_ids"])[0].to(dtype=weight_dtype)
                # a = inversion.get_text_embedding("")
                # print(a.shape)
                #读取Vsrc得到的embedding
                backward_embeds_dict_name = "_%s.bin"%(str(args.max_train_steps))
                backward_placeholder_token = ""
                #backward_embeds_dict_path = os.path.join(model_id, backward_embeds_dict_name)
                #backward_learned_embeds_dict = torch.load(backward_embeds_dict_name)
                backward_learned_embeds_dict = torch.load("/home/dingkaixin/improved/output/_500.bin")
                # print(backward_learned_embeds_dict.shape)
                print("backward_learned_embeds_dict")
                print(backward_learned_embeds_dict[''].shape)
                import numpy as np
                tensor = backward_learned_embeds_dict['']
                tensor = tensor[:, :768]
                backward_learned_embeds_dict[''] = tensor
                text_encoder = CLIPTextModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder", revision=False)
                pivot_turning_inversion.load_multitoken_tokenizer(tokenizer, text_encoder, backward_learned_embeds_dict, backward_placeholder_token)
                
                for file in os.listdir(args.content_image_dir):
                    image_path = os.path.join(args.content_image_dir, file)
                image_gt = pivot_turning_inversion.load_512(image_path)
                image_latent = inversion.image2latent(image_gt)
                text_embeddings = inversion.get_text_embedding("s2")
                ...
                print("batch[""input_ids""]")
                print(batch["input_ids"].shape)
                #reshape_batch_id = np.reshape(batch["input_id"], (1, 77, 768))
                encoder_hidden_states, latent_cur = inversion.mynew_null_optimization_path(
                    reversed_latent_list, #Vsrc 的embedding
                    batch["input_ids"],#reshape_batch_id,
                    num_inner_steps=50,
                    epsilon=1e-5,
                    guidance_scale=7.5,
                    num_ddim_steps=50
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(args.output_dir, concept_folder_name)
    ...
    #GetVsrc(args)
    # placeholder_token = ""
    #output_dir_src = GetVsrc(tokenizer, text_encoder, unet, vae, noise_scheduler, placeholder_token="")

    # args.placeholder_token = "s2"
    output_dir = multi_concept_inversion(tokenizer, text_encoder, unet, vae, noise_scheduler, placeholder_token="s2") #get V reference and store it in _s2__.bin

    #args.output_dir = "./output/concept_09_16_2023_1935"
