...
def multi_concept_inversion():
    args = parse_args()
    src_embedding_list, src_reversed_latent_list = train_src.train_src_embedding(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                mask = torch.where(timesteps % 100 < 50, torch.tensor(1,device=latents.device), torch.tensor(0,device=latents.device))
                # print("mask ", mask )
                masks.append(mask)
                ...
                # src_pred = unet(noisy_latents, timesteps, src_embedding_list[timesteps // 100]).sample
                src_pred = unet(noisy_latents, timesteps, src_embedding_list[-1]).sample
                ...
                l2_target = target + args.guidance_scale/(args.guidance_scale + 1) * (src_pred - target)
    ...
    return args, src_reversed_latent_list, src_embedding_list


if __name__ == "__main__":
    args, src_reversed_latent_list, src_embedding_list = multi_concept_inversion()
    ...
    gen_image.gen_image(
        pretrained_model_name_or_path=args.pretrained_model_name_or_path,
        output_img_path_name=args.output_image_path + "/__out__"+ str(args.guidance_scale) +"_"+ time_smp + ".png",
        backward_prompts=["", "<s2>"],
