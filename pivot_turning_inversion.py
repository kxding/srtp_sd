...
def main(model_id, image_path, step_list, prompt_name_backward, forward_prompt, backward_prompts, out_img_save_path, content_name_pre, cross_attention_injection_ratio, self_attention_injection_ratio, pretrained_model_name_or_path):
    ...
    for train_step in step_list:
        ...
        #ldm_stable = Get_ldm_stable(pretrained_model_name_or_path, scheduler=scheduler, tokenizer=tokenizer, text_encoder=text_encoder)
        ...
        #通过遍历内容图像路径下的图像文件，并根据指定的训练步骤和其他参数，调用 `main` 函数执行多概念反转，并将结果保存到指定的输出路径中。
    ...
    for i in range(len(input_img_list)):
        ...
        o1 = out_img_save_path + "output_%s.jpg"%(i)
        o2 = out_img_save_path + "third_%s.jpg"%(i)
        output_img_list[i].save(o1)
        third_img_list[i].save(o2)
