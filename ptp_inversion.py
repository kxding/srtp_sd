...
MAX_NUM_WORDS = 77
LOW_RESOURCE=False
#context is same as text embedding in CLIP
def default_score_estimation(model,latents,context,t,guidance_scale,do_classifier_free_guidance):
    ...
    if do_classifier_free_guidance:
        #在分类器自由引导的情况下，我们希望生成的噪声能够受到来自文本指导的影响，但仍然保留一些与输入无关的随机性。因此，`noise_pred_uncond` 表示与输入无关的噪声预测部分，`noise_pred_text` 表示来自文本指导的噪声预测部分。
...
class Inversion(DiffusionPipeline):
    ...
    def get_noise_pred_single(self, latents, t, context):
        ...
        # print("context")
        # print(context.shape)
        if context.shape == (1,77, 768) and latents.shape == (1, 4, 64, 64):
    ...
    @torch.no_grad()
    def ddim_loop(self, latent,num_ddim_steps):#差分扩散
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon,guidance_scale=7.5,num_ddim_steps=50):
        ...
        for i in range(num_ddim_steps):
            ...
            for j in range(num_inner_steps):
                ...
                loss = nnf.mse_loss(latents_prev_rec, latent_prev)#每一步都将预测的前一步和 实际前一步进行比较
                optimizer.zero_grad()
                loss.backward() #反向传播
                optimizer.step()
                loss_item = loss.item()
    ...
    def mynew_null_optimization_path(self, latent_list, cond_embeddings, num_inner_steps, epsilon, guidance_scale=7.5, num_ddim_steps=50):
        ...
        for i in range(num_ddim_steps):
            ...
            cond_embeddings.required_grad = True
            optimizer = Adam([cond_embeddings], lr = 1e-2 * (1. - i / 100.))
            latent_prev = latent_list[len(latent_list) - i - 2]
            t = self.model.scheduler.timesteps[i]
            with torch.no_grad():
                if latent_cur.shape == (1, 4, 64, 64) and uncond_embeddings.shape == (1, 77, 768):
                    noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, uncond_embeddings)
            for j in range(num_inner_steps):
                l1 = torch.reshape(latent_cur, (1, 4, 64, 64))
                c1 = torch.reshape(cond_embeddings, (1, 77, 768))
                noise_pred_cond = self.get_noise_pred_single(l1, t, c1)
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
                latents_prev_rec = self.prev_step(noise_pred, t, latent_cur)
            ...
            with torch.no_grad():
                context = torch.cat([uncond_embeddings, cond_embeddings])
                latent_cur = pivot_turning_inversion.inversion.get_noise_pred(latent_cur, t, False, context)
        return cond_embeddings, latent_cur
    ...
    def null_optimization_path(self, latent_list,cond_embeddings,num_inner_steps, epsilon, guidance_scale=7.5,num_ddim_steps=50):
        ...
        for i in range(num_ddim_steps):
            ...
            # context = torch.cat([uncond_embeddings, cond_embeddings])
            # print(context.shape) #[2, 77, 768]
            # print("cond_embedding:")
            # print(cond_embeddings.shape)
                
            for j in range(num_inner_steps):
                #uncond_embeddings = uncond_embeddings.repeat(2,1,1)
                noise_pred_uncond = self.get_noise_pred_single(latent_cur, t, uncond_embeddings)
                # with torch.no_grad():
                #     noise_pred_cond = self.get_noise_pred_single(latent_cur, t, context)
                # cond_shape = cond_embeddings.size()
                # context = context.view(cond_shape)
                #noise_pred = noise_pred_uncond + guidance_scale * (context - noise_pred_uncond)
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
                latents_prev_rec = self.prev_step(noise_pred, t, latent_cur)
                loss = nnf.mse_loss(latents_prev_rec, latent_prev)
                optimizer.zero_grad()
...
def text2image_ldm_stable(
    ...
    text_embeddings = model.text_encoder(text_input.input_ids.to(model.device))[0]
    max_length = text_input.input_ids.shape[-1] #输入文本的最大长度
    if uncond_embeddings is None:
        uncond_input = model.tokenizer(
            [""] * batch_size, padding="max_length", max_length=max_length, return_tensors="pt"
        )
