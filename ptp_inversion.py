...
class Inversion(DiffusionPipeline):
    ...
    def backward_diffusion_ptp(self,
        ...
        if return_all:
            all_latents = [latents]
        # my_init_context = torch.zero([uncond_embeddings[0].shape[0], uncond_embeddings[0].shape[1]+text_embeddings.shape[1]])
        if uncond_embeddings_ is None:
            my_init_context = torch.concat([uncond_embeddings[0], text_embeddings])
        else:
            my_init_context = torch.concat([uncond_embeddings_[0], text_embeddings])
